...
- 将 Nixpkgs 的海量软件包移植到龙架构平台，提供龙架构的 [NixOS 安装镜像](/zh/installation)
- 提供龙架构的 Nixpkgs 软件包的[二进制缓存](/zh/usage)，免去本地构建带来的各种麻烦
- 搭建桥梁，让 Nix 社区能够参与和贡献龙芯软件生态的发展

## 当前进展

目前，Nix4Loong 项目正在积极推进 Nixpkgs 软件包的龙架构适配工作，向上游合入了大量修改，并已成功构建出可正常运行的 NixOS 的安装镜像。我们的目标是在 NixOS 25.11 版本发布之前（约 2025 年 11 月），将适配龙架构的修改全部合并到上游，同时确保相当数量的软件包能够正常构建和运行。
