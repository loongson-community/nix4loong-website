...
强烈建议将您设备的固件更新至最新版本。您可以在 [loongfans.cn 设备列表](https://loongfans.cn/devices)中找到对应设备，前往其详情页的「文件下载」一节获取固件。
</Callout>
...
如果您不方便使用安装镜像（例如远程 VPS等环境），可使用 [nix4loong-infect](https://github.com/Nix4Loong/nix4loong-infect) 将一台正在运行的龙架构 Linux 主机（AOSC OS、Arch Linux for Loong64 等）**原地替换**为 NixOS。它是 [elitak/nixos-infect](https://github.com/elitak/nixos-infect) 的龙架构移植版本，默认使用 Nix4Loong 的 Nix 安装程序、Nix Channel 和二进制缓存。
